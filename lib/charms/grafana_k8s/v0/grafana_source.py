...
from ops.charm import CharmBase, CharmEvents, RelationBrokenEvent, RelationChangedEvent, RelationJoinedEvent
...
class GrafanaSourceConsumer(ConsumerBase):
    ...
    def add_source(self, data: dict, rel_id=None) -> None:
        ...
        """
        logger.info("GETTING RELATION FOR: {}".format(self.name))
        rel = self.framework.model.get_relation(self.name, rel_id)
        logger.info("GOT {}".format(rel))
        rel_id = rel_id if rel_id is not None else rel.id

        logger.info("CHARM IS: {}".format(self.charm))
        logger.info("CHARM APP IS: {}".format(self.charm.app))
        logger.info("CHARM APP NAME IS: {}".format(self.charm.app.name))
    ...
    def _set_sources(self, event: RelationJoinedEvent):
        ...
        event.relation.data[self.charm.app]["sources"] = json.dumps(
            self.stored.sources[rel_id]
        )

    def _update_sources(self, data, rel_id):
...
class GrafanaSourceProvider(ProviderBase):
    ...
    def on_grafana_source_relation_changed(self, event: RelationChangedEvent) -> None:
        ...
        if self._stored.sources.get(rel.id, None) and \
                len(set(data.items()) ^ set(self._stored.sources[rel.id].items())) == 0:
            logger.info("Relation data unchanged. DOING NOTHING")
            return
        else:
            logger.info("STORED: {}".format(self._stored.sources.get(rel.id, None)))
            logger.info("DATA: {}".format(dict(data)))
