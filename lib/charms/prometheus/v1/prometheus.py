...
class PrometheusProvider(ProviderBase):
    ...
    def _on_scrape_target_relation_changed(self, event):
        ...
        if job_name:
            job_name = "{}_{}".format(job_name, unique_name)
        else:
...
class PrometheusConsumer(ConsumerBase):
    ...
    def add_endpoint(self, address, port=80, rel_id=None, job_name=None):
        ...
        self._update_targets(targets, rel_id, job_name)
    ...
    def _update_targets(self, targets, rel_id, job_name):
        ...
        rel.data[self._charm.app]["job_name"] = json.dumps(job_name)
